<%--
		...
		3. Servlet VS JSP
			...
			=> Servlet은 자바파일 => 수정시마다 컴파일을 해야한다
				...
				3. CSS를 구사하기 어렵다 => 개발자.. (HTML, JS, CSS, Oracle...)
			=> 단점을 보완 : JSP (HTML을 쉽게 다루게 만든다)
			=> 장점
				서블릿 보다는 쉽고 작성하기 빠르다
				HTML과 Java가 분리되어 있다
				------------ 분업해서 사용이 가능
				=> 컴파일 하지 않고 바로 이동이 가능하다
		...
		5. JSP 문법
			...
			JSP = > 24page
			---------------
				동적 컨텐츠 => 한개의 파일 안에서 데이터를 변경 할 수 있다
				자바언어를 사용한다
				자바 / HTML이 분리되어 있다
				JSP는 항상 => 서블릿으로 변경 후에 실행
							  ------------------------- 톰캣에 의해 변경
				JSP는 화면에 출력할 내용의 메소드인데 들어가는 내용을 코딩한다
				--- 클래스가 아니고 메소드 영역이다
				_jspService() {
					---------------
					     JSP
