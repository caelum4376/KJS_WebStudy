...
<%--
	...
	주요메소드
			...
			isNew() : ID가 할당이 된것인지 여부 확인
				=> 장바구니
			invalidate() : session에 저장된 모든 내용을 지운다 (로그아웃)
...
<body>
	<h1>HttpServletRequest(session) : 177page</h1>
	<table class="table">
		<tr>
			<th width=20% class="text-center">클래스명</th>
			<td width=80%>HttpSession(request로 생성이 가능 : request.getSession())</td><br>
		...
		<tr>
			...
			<td width=80%>
				<ul>
					...
					<li>전역변수로 사용이 가능 (모든 JSP에서 사용)</li>
