...
<%--
	_jspService() { => 브라우저에서 실행하는 메소드 => service
		==============
			JSP
		==============
	}
...
%>
<%--
		실행시에 JSP는 톰캣에 의해 servlet 클래스로 변경
		post.jsp => post_jsp.java
				...
				1) 구성요소
					=> AWS : Java / Tomcat => 프로젝트
					server : Web Server => 아파피 / IIS
		...
		1) 지시자
			...
			= include : JSP안에 특정위치 다른 JSP를 포함
				<%@ include file="a.jsp">
			= taglib : 자바의 제어문, 변수선언, String => 태그로 만들어서 제공
				...
				<c:forEach var=i begin=0 end=10 step1>
					<ul>
					</ul>
				</c:forEach>
		1-1) 스크립트릿
		...
		2) 내장객체
			...
			= out***
			= pageContext*** : 페이지 흐름
				<jsp:include> <jsp:forward>> => JSP액션 태그로 변경
			= config(web, xml)
			= exception*** (try~catch)
			= page*** (this)
		3) JSP 액션 태그
