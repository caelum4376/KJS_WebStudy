...
<%--
	...
	----------------------------------------------------------
	218page 빈을 이용한 JSP 파일작성
	------------------------------------
		jsp 액션태그
		=> 화면이동, 	화면연결, 	값전송, 	객체생성, 	멤버변수에 값 주입, 	멤버변수의 값 읽기
			  |		<jsp:include>	  |		  <jsp:useBean>			|				<jsp:getProperty>
		<jsp:forward>			<jsp:param>						<jsp:setProperty>
	------------------------------------
		...
		속성
			...
			scope : 사용범위
				...
				== application => 프록그램 종료시까지 유지				
			<jsp:useBean id="a" class="A">
								----- 패키지명.클래스명 => Class.forName()
			=> 자바 변경
				A a = new A();
	------------------------------------
		<jsp:setProperty> : 변수 에 값을 설정 (쓰기)
		=> setter
		name :  객체명
		property : 변수
		value : 값을 설정
